...
JOIN ratings r
	ON m.id = r.movie_id
WHERE date_published BETWEEN (2017-03-01 AND 2017-03-31) AND country = 'USA' AND total_votes > 1000
GROUP BY genre;
...
JOIN ratings r
	ON m.id = r.movie_id
GROUP BY country
HAVING country = 'Germany' OR country = 'Italy'
...
WHERE avg_rating > 8
GROUP BY name_of_director, genre
...
SELECT 	
	t1.*, 
    RANK() OVER (ORDER BY t1.actor_avg_rating DESC, t1.total_votes DESC) AS actor_rank
FROM 	
	(SELECT 
		n.id AS actor_id, 
...
JOIN ratings r 
	ON m.id = r.movie_id
WHERE LOWER(m.country) like '%india%' AND LOWER(m.languages) like '%hindi%'AND rm.category = 'actress'
GROUP BY n.id, n.name
HAVING movie_count >= 3) AS t1;
...
SELECT 
    ...
    SUM(total_votes) ,COUNT(m.id) AS movie_count ,
